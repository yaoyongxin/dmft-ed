!include "chi0_lattice_ave.f90"
include "chern_marker3_NF.f90"
program ed_nano_isoc
  ...
  logical                                         :: phsym
  logical                                         :: leads
  logical                                         :: kinetic,trans,jbias,jrkky,chi0ij,chern
  logical                                         :: para
  logical                                         :: self_zero
  ...
  !Interface for the chi0 routine
  interface !ed_get_chi0ij(Gijmats,Nvec,Kvec,Qvec,e1,e2,e3)
  ...
  interface
     subroutine get_local_chern(Hij,Smats)
       ...
       complex(8), intent(in)                                  :: Smats(:,:,:,:,:,:) ![Nlat*Nspin*Nspin,Norb,Norb,Lreal][Nlat]
  ...
  call parse_input_variable(chern,"chern",finput,default=.false.)
  ...
  call parse_input_variable(self_zero,"self_zero",finput,default=.false.)
  !call parse_input_variable(Nx,"Nx",finput,default=10,comment="Number of points for each side of the lattice")
  ! read input

  call parse_input_variable(Nk_chi0,"Nk_chi0",finput,default=30,comment="Number of k points for 2d BZ integration")
  call parse_input_variable(Nq_chi0,"Nq_chi0",finput,default=30,comment="Number of qpoints in the 2d BZ for chi0")
  ...
  !Here postprocessing for chern marker (topological features)
  if(chern)then
     ...
     enddo
     write(*,*), Nlat,Nspin, Norb, Lmats
     call get_local_chern(Hij,Smats)
     stop
  endif
  ...
  if(chi0ij)then
     ...
     if(leads)then
        call set_hyb()
        call dmft_set_Gamma_matsubara(hyb_mats) !needed for dmft_gij_matsubara
     endif
     !
     ...
     enddo
     !     
     ! set self-energy to zero (non-interacting susceptibility)
     if(self_zero)then
        Smats = zero
     else 
        call read_sigma_mats(Smats_ineq)
     ...
     write(*,*) "here is the Gij computed"
     ...
     !ed_get_chi0ij(Gijmats,Nvec,Kvec,Qvec,e1,e2,e3)

     e1= 3.d0/2.d0*[1.d0 ,  1.d0/sqrt(3.d0)]
     e2= 3.d0/2.d0*[1.d0 , -1.d0/sqrt(3.d0)]
     !>DEBUG
     !call ed_get_chi0ij(Gijmats,Nvec=[Nlat,Nlat],e1=e1,e2=e2)
     !<DEBUG
     !call ed_get_chi0ij(Gijmats,Nvec=[Nlat,Nlat],Kvec=[Nk_chi0,Nk_chi0],Qvec=[Nq_chi0,Nq_chi0], e1=e1, e2=e2)
     !
...
contains
  ...
  subroutine save_sigma_mats(Smats)
    ...
    do ispin=1,Nspin
       do iorb=1,Norb
          ...
          call splot("LSigma"//trim(suffix),wm,Smats(:,ispin,ispin,iorb,iorb,:))
       enddo
    enddo
  ...
  subroutine save_sigma_real(Sreal)
    ...
    do ispin=1,Nspin
       do iorb=1,Norb
          ...
          call splot("LSigma"//trim(suffix),wr,Sreal(:,ispin,ispin,iorb,iorb,:))
       enddo
    enddo
  ...
  subroutine ed_transport(Gret)
    ...
    !>>>>>>> 518b0d97be48500de6123c9b7bf1f2db8693f9cf
