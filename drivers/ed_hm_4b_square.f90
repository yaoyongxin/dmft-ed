program ed_hm_4bands
  ...
  if(Norb/=4)stop "Wrong setup from input file: Norb=4"
  Nso=Nspin*Norb
  Nlso=Nso
...
contains
  ...
  function hk_model(kpoint,Nlso) result(hk)
    ...
    hk(3,3) = -2d0*ts*(cos(kx)+cos(ky))
    hk(4,4) = -2d0*ts*(cos(kx)+cos(ky))
    !
  ...
  subroutine build_hk(file)
    ...
    call TB_solve_model(hk_model,Nlso,kpath,Nkpath,&
         colors_name=[red1,green1,blue1,orange1],&
