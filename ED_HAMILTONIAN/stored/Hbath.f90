           ...
           do iorb=1,Norb
              do jorb=1,Norb
                 ...
                 if (Jcondition)then
                    ...
                    htmp = conjg(dmft_bath%h(1,1,iorb,jorb,kp))*sg1*sg2
                    !
                    select case(MpiStatus)
                    case (.true.)
                 ...
                 if (Jcondition)then
                    ...
                    call cdg(alfa,k1,k2,sg2)
                    j = binary_search(H%map,k2)
                    htmp = conjg(dmft_bath%h(Nspin,Nspin,iorb,jorb,kp))*sg1*sg2
                    !
                    select case(MpiStatus)
                    case (.true.)
                       call sp_insert_element(MpiComm,spH0,htmp,i,j)
                 ...
                 do iorb=1,Norb
                    do jorb=1,Norb
                       ...
                       if(Jcondition)then
                          ...
                          call cdg(alfa,k1,k2,sg2)
                          j = binary_search(H%map,k2)
                          htmp = conjg(dmft_bath%h(ispin,jspin,iorb,jorb,kp))*sg1*sg2
                          !
                          select case(MpiStatus)
                          case (.true.)
                             call sp_insert_element(MpiComm,spH0,htmp,i,j)
